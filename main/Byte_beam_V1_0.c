...
// New thread
nwy_osiThread_t *new_thread = NULL;
...
nwy_osiSemaphore_t *new_thread_semaphore = NULL;
...
//    } while((parse_device_config_file() < 0) && (config_data_present_in_nwy == 0));
    ...
    // Initializing with a count of 1 to allow locking
    new_thread_semaphore = nwy_semaphore_create(1, 0);
